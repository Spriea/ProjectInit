...
+ (void)load
{
    ...
    NSDictionary *colorDict = @{
                                NSForegroundColorAttributeName : Color(@"#3F4BDD")
                                };
...
- (void)setUpChildViewController{
    ...
    [self addChildViewController:[[SKBaseNav alloc] initWithRootViewController:[NSClassFromString(@"SKModalVC") new]]];
    [self addChildViewController:[[SKBaseNav alloc] initWithRootViewController:[NSClassFromString(@"SKResourceVC") new]]];
    [self addChildViewController:[[SKBaseNav alloc] initWithRootViewController:[NSClassFromString(@"SKDiscoverVC") new]]];
...
{
    NSArray *titleArr = @[@"首页",@"模板",@"资源",@"发现",@"我的"];
    NSArray *imgArr = @[@"info",@"record",@"map",@"mine",@"mine"];
    NSArray *selImgArr = @[@"info_sel",@"record_sel",@"map_sel",@"mine_sel",@"mine"];
    self.tabBar.tintColor = Color(@"#3F4BDD");
    if (@available(iOS 10.0, *)) {
        self.tabBar.unselectedItemTintColor = k3Color;
    } else {
